...
test("profile declaration semantics reject missing and incompatible fields", () => {
  const configured = {
  ...
  assert.match(profileDeclarationErrors({ ...configured, profile: undefined })[0], /must declare a profile/);
...
test("official manifests declare deliberate reusable profile contracts", async () => {
  const expected = {
    "app-store-connect": { type: "app-store-connect-api-key", requiredFields: ["issuerId", "keyId", "privateKeyFile"] },
    cloudflare: { type: "cloudflare-api-token", requiredFields: ["apiToken", "accountId"] },
    "google-ads": { type: "google-ads-oauth", requiredFields: ["developerToken", "clientId", "clientSecret", "refreshToken"] },
    "google-analytics": { type: "google-credential-json", requiredFields: ["credentialJson", "measurementRegion"] },
    ...
    "google-tag-manager": { type: "google-credential-json", requiredFields: ["credentialJson"] },
    heroboard: { type: "heroboard-api-key", requiredFields: ["apiKey"] },
    "image-generator": { type: "google-ai-api-key", requiredFields: ["apiKey"] },
  };
