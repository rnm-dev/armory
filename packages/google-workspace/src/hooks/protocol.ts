export type HookInput = { protocolVersion: 1; type: "input"; operation: "configure" | "verify";
  package: { id: string; version: string; dir: string; home: string }; platform: { os: "darwin" | "linux"; arch: "x64" | "arm64" };
  configuration?: Record<string, string> };
export async function readInput(): Promise<HookInput> {
  let input = ""; for await (const chunk of process.stdin) { input += chunk; if (Buffer.byteLength(input) > 2 * 1024 * 1024) throw new Error("input too large"); }
